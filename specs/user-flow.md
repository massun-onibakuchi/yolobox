# Scheduled Agent Runner User Flow

## Register Task

1. User edits `.automation/scheduled-agent-runner/config.toml` with Telegram and model defaults.
2. User runs `bin/scheduled-agent-runner schedule ...`.
3. The tool validates the schedule, stores the task, and computes the first run time.

## Run Tasks

1. User runs `bin/scheduled-agent-runner run`.
...
1. User runs `bin/scheduled-agent-runner status`.
2. The tool prints current tasks, next run, session mode, and queue depth.

## Unregister Task

1. User runs `bin/scheduled-agent-runner remove --task <id>`.
...
1. User runs `bin/scheduled-agent-runner pause --task <id>`.
2. The tool marks the task disabled without deleting history.
3. User runs `bin/scheduled-agent-runner resume --task <id>`.
4. The tool marks the task enabled and recalculates its next run from the current time.

## Edit Task

1. User runs `bin/scheduled-agent-runner edit --task <id> --prompt <text>`.
